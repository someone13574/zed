...
cfg_if! {
    if #[cfg(any(unix, target_os = "hermit"))] {
        ...
        /// A resource with a raw source handle.
        pub trait AsRawSource {
            /// Returns the raw source handle.
            fn raw(&self) -> usize;
        }

        impl<T: AsRawSource + ?Sized> AsRawSource for &T {
            fn raw(&self) -> usize {
                (**self).raw()
            }
        }

        impl AsRawSource for usize {
            fn raw(&self) -> usize {
                *self
            }
        }

        /// A resource with a borrowed source handle.
        pub trait AsSource {
            /// Returns the borrowed source handle.
            fn source(&self) -> usize;
        }

        impl<T: AsRawSource + ?Sized> AsSource for T {
            fn source(&self) -> usize {
                self.raw()
            }
        }

...
fn configure_wasm_build(config: &mut cc::Build) {
    let Ok(wasm_headers) = env::var("DEP_TREE_SITTER_LANGUAGE_WASM_HEADERS") else {
        println!(
            "cargo:warning=DEP_TREE_SITTER_LANGUAGE_WASM_HEADERS is not set; skipping wasm stdlib shim sources"
        );
        return;
    };
    let Ok(wasm_src) = env::var("DEP_TREE_SITTER_LANGUAGE_WASM_SRC").map(PathBuf::from) else {
        println!(
            "cargo:warning=DEP_TREE_SITTER_LANGUAGE_WASM_SRC is not set; skipping wasm stdlib shim sources"
        );
        return;
    };
